...
public class Huffman {
    ...
    public void writeCompressedFile(String path) throws IOException {
        readNewFile(path);
    ...
    public void decodeCompressedFiled(String path) throws IOException {
        FileInputStream fileInputStream = new FileInputStream(path);
        DataInputStream dataInputStream = new DataInputStream(fileInputStream);
        byte[] compressedData;

        for(int i = 0; i < frequency.length; i ++){
            frequency[i] = dataInputStream.readInt();
        }
        ...
        byte[] decompressedText = new byte[Arrays.stream(frequency).sum()];
    ...
    public void printFrequencyArray(){
        ...
        for(int i = 0; i < frequency.length; i ++){
            if(frequency[i] != 0){
                System.out.format("%-8s%-8s%n", (char)i, frequency[i]);
            }
        }
    }
